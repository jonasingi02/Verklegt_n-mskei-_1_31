...
class VoyageUI:
    ...
    def input_prompt(self):
        ...
        while True:
            ...
            if command == "1":
                assignments = self.logic_wrapper.get_voyage_personnel_assignments()
                for assignment in assignments: 
                    voyage = assignment["vinnuverð"]
                    print(f"vinnuferð: {voyage.id}, dagsetning: {voyage.date}, flugvél: {voyage.plane}, flugvöllur: {voyage.airport}")
                    print("Starfandi flugmenn:")
                    for pilot in assignment['flugmenn']:
                        print(f" - {pilot}")
                    print("Starfandi flugáhöfn:")
                    for attendant in assignment['flugáhöfn']:
                        print(f" - {attendant}")
                    print()
